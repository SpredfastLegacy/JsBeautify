...
Output options:
 ...
 -m,  --max-preserve-newlines      maximum line breaks to preserve.
...
def main():
    ...
    try:
        opts, args = getopt.getopt(argv, "s:c:o:djbkil:xhtf:m", ['indent-size=','indent-char=','outfile=', 'disable-preserve-newlines',
                                                          'jslint-happy', 'brace-style=',
                                                          'keep-array-indentation', 'indent-level=', 'unescape-strings', 'help',
                                                          'usage', 'stdin', 'eval-code', 'indent-with-tabs', 'keep-function-indentation', 'max-preserve-newlines='])
    ...
    for opt, arg in opts:
        ...
        if opt in ('--keep-function-indentation','-f'):
            ...
            js_options.max_preserve_newlines = 2
        elif opt in ('--stdin', '-i'):
            file = '-'
